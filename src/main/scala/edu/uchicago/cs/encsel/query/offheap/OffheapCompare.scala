...
import java.io.File
import edu.uchicago.cs.encsel.query.offheap
...
object Offheap extends App {
  ...
  def test(name: String, pred: Predicate): Unit = {
    ...
    for (i <- 0 until repeat) {
      val prof = ParquetReaderHelper.profile(new File("/Users/chunwei/research/tpch/offheap/lineitem.parquet").toURI, new EncReaderProcessor() {
