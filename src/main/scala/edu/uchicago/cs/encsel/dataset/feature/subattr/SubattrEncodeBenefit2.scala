...
object SubattrEncodeBenefit2 extends FeatureExtractor {
  ...
  override def extract(col: Column, input: InputStream, prefix: String): Iterable[Feature] = {
    ...
    if(Files.exists(Paths.get(subtableFile))) {
      ...
      Iterable(
        new Feature("SubattrStat", "subattr_benefit_st", childrenSize / originalSize),
        new Feature("SubattrStat", "subattr_benefit_st_plain", childrenSize / pSize)
      )
    } else {
      Iterable()
    }
